...
def makeGenerator(data_shape, noise_vect_size):
    ...
    generator.add(Conv2DTranspose(channels, (3, 12), padding='same', activation = 'sigmoid'))
    generator.add(Conv2DTranspose(channels, (3, 12), strides = (1,1), padding='same', activation = 'sigmoid'))
...
def makeDiscriminator(data_shape):
    ...
    discriminator.add(Conv2D(64, (3, 12), padding='same', input_shape=(data_shape), activation = 'sigmoid'))
    # discriminator.add(MaxPooling2D(pool_size=(2, 2)))
