...
def makeGenerator(data_shape, noise_vect_size):
    ...
    # generator.add(Dense(512, activation = 'sigmoid', input_dim=noise_vect_size, kernel_initializer=initializers.RandomNormal(stddev=0.02)))
    ...
    #nikhil's crazy model
    generator.add(Dense(data_size, activation = 'relu', input_dim=noise_vect_size, kernel_initializer=initializers.RandomNormal(stddev=0.02)))

    fractionx = fractiony = 2
    ...
    generator.add(Reshape(new_shape))
    generator.add(Dropout(.1))
    generator.add(Conv2DTranspose(32, (3, 3), padding='same'))
    generator.add(LeakyReLU(0.1))

    generator.add(UpSampling2D((fractionx, fractiony)))
    generator.add(Conv2DTranspose(channels, (3, 3), padding='same', activation = 'relu'))
    generator.add(LeakyReLU(0.1))
