...
def makeGenerator(data_shape, noise_vect_size):
    ...
    generator.add(Dense(100, activation = 'sigmoid', input_dim=noise_vect_size, kernel_initializer=initializers.RandomNormal(stddev=0.02)))
    ...
    generator.add(Conv2DTranspose(32, (24, 1), padding='same', input_shape = data_shape))
    generator.add(LeakyReLU(0.25))
    generator.add(Conv2DTranspose(channels, (1, 2), padding='same', input_shape = data_shape))
    generator.add(LeakyReLU(0.25))
    #generator.add(Conv2DTranspose(32, (2, 1), padding='same', input_shape = data_shape))
    #generator.add(LeakyReLU(0.25))
    #generator.add(Conv2DTranspose(channels, (2, 1), padding='same', activation = 'relu'))
...
def makeDiscriminator(data_shape):
    ...
    discriminator.add(Conv2D(64, (24, 4), padding='same', input_shape=(data_shape), activation = 'relu'))
    discriminator.add(Conv2D(64, (1, 4), input_shape=(data_shape), padding='same', activation = 'relu'))
    #discriminator.add(Conv2D(64, (4, 1), input_shape=(data_shape), padding='same', activation = 'relu'))
